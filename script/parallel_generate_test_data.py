...
NUM_ACCOUNTS = 5
NUM_INCOME_RECORDS = 1000
NUM_RANDOM_EXPENSES = 6000
NUM_GOALS = 5
NUM_BUDGETS = 5  # e.g., Food, Transport, Entertainment, Shopping
NUM_RECURRING_RULES = 8

# Date range for expenses/income (relative to today)
DATA_START_MONTHS_AGO = 60
...
BUDGET_CATEGORIES = [
    ...
    "Travel",
    "Personal Care",
]  # Categories to create budgets for
RECURRING_ITEMS = [
    {
        "name": "Rent/Mortgage",
        "category": "Housing",
    ...
    },
    {
        ...
        "amount": 120,
        "frequency": "monthly",
    },  # Example semi-annual might be harder to script simply
    {
        "name": "Cloud Storage",
        "category": "Subscriptions",
        "amount": 9.99,
...
def post_data(endpoint, data, item_type, item_desc):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(
                f"Failed to create {item_type} ({item_desc}): {response.status_code} - {response.text}"
            )
            return None
    except requests.exceptions.RequestException as e:
...
def generate_accounts(user_id, num_accounts):
    ...
    for acc_type in common_types:
        if stop_flag.is_set() or created_count >= num_accounts:
            break
        ...
        balance_date = (
            datetime.date.today() - relativedelta(months=DATA_START_MONTHS_AGO)
        ).isoformat()
        start_balance = (
            round(random.uniform(500, 5000), 2)
            if acc_type != "Credit Card"
            else round(random.uniform(-1000, 0), 2)
        )
...
def generate_single_income(user_id, start_date, end_date, account_ids):
    ...
    account_id = (
        random.choice(account_ids) if account_ids and random.random() < 0.7 else None
    )  # 70% chance to link
...
def generate_income_records_concurrently(
    user_id, num_records, start_date, end_date, account_ids
):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            executor.submit(
                generate_single_income, user_id, start_date, end_date, account_ids
            )
...
def generate_single_expense(user_id, start_date, end_date, account_ids):
    ...
    account_id = (
        random.choice(account_ids) if account_ids and random.random() < 0.5 else None
    )  # 50% chance to link
...
def generate_expenses_concurrently(
    user_id, num_records, start_date, end_date, account_ids
):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=8
    ) as executor:  # More workers for expenses
        futures = [
            executor.submit(
                generate_single_expense, user_id, start_date, end_date, account_ids
            )
            for _ in range(num_records)
        ]
        concurrent.futures.wait(futures)
...
def generate_recurring_rules(user_id, num_rules, account_ids):
    ...
    for item in items_to_use:
        if stop_flag.is_set():
            break
        start_date = (
            datetime.date.today() - relativedelta(months=random.randint(6, 18))
        ).replace(day=random.randint(1, 15))
        end_date = (
            start_date + relativedelta(years=random.randint(1, 3))
            if random.random() < 0.3
            else None
        )  # 30% have end dates
        account_id = (
            random.choice(account_ids)
            if account_ids and random.random() < 0.8
            else None
        )
        ...
        payload = {
            ...
            "name": item["name"],
            "amount": round(
                item["amount"] * random.uniform(0.9, 1.1), 2
            ),  # Slight variation
...
def generate_budgets(user_id, num_budgets):
    ...
    created_count = 0
    temp_budgets = []
    categories_to_use = random.sample(
        BUDGET_CATEGORIES, min(num_budgets, len(BUDGET_CATEGORIES))
    )
    ...
    for category in categories_to_use:
        ...
        start_date = (
            datetime.date.today() - relativedelta(months=random.randint(1, 3))
        ).replace(day=1)
        ...
        result = post_data(
            "/budgets", payload, "Budget", f"{category} ${amount_limit}/{period}"
        )
        if result:
...
def generate_goals(user_id, num_goals):
    ...
    for name in goals_to_use:
        ...
        current_amount = round(
            target_amount * random.uniform(0.1, 0.8), 2
        )  # Partially funded
        target_date = (
            datetime.date.today() + relativedelta(months=random.randint(6, 24))
        ...
        result = post_data(
            "/goals", payload, "Goal", f"{name} (${current_amount}/${target_amount})"
        )
        if result:
...
def main():
    ...
    print(
        f"Data generation date range: {start_date.isoformat()} to {end_date.isoformat()}"
    )
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=14) as executor:
            ...
            futures.append(
                executor.submit(
                    generate_income_records_concurrently,
                    user_id,
                    NUM_INCOME_RECORDS,
                    start_date,
                    end_date,
            ...
            futures.append(
                executor.submit(
                    generate_expenses_concurrently,
                    user_id,
                    NUM_RANDOM_EXPENSES,
                    start_date,
                    end_date,
            ...
            futures.append(
                executor.submit(
                    generate_recurring_rules, user_id, NUM_RECURRING_RULES, account_ids
                )
            )
